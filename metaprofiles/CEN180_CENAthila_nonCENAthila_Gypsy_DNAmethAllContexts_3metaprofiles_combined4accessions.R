...
ChIP_ranLocMats <- lapply(seq_along(ChIP_ranLocMats), function(w) {
  tmp <- mclapply(seq_along(ChIP_ranLocMats[[w]]), function(x) {
    if(length(chrName) > 1) {
      do.call(rbind, ChIP_ranLocMats[[w]][[x]])
    } else {
      ChIP_ranLocMats[[w]][[x]][[1]]
    }
  }, mc.cores = length(ChIP_ranLocMats[[w]]))
  do.call(rbind, tmp)
})
...
scale_x_discrete(breaks = c(1,
                            ((upstream-1000)/binSize)+1,
                            (dim(summaryDFfeature_ChIP[[2]])[1]/length(contextNames))-((downstream-1000)/binSize),
...
ggsave(paste0(plotDir,
              ...
              "_avgProfiles_around",
              "_CEN180_ranLoc_CENAthila_nonCENAthila_Gypsy_in_",
